...
warnings.filterwarnings(
    "ignore", category=RuntimeWarning, message=".*coroutine.*expire_cache.*"
)

st.set_page_config(
    page_title="Treasury Auction Analytics", page_icon="🏛️", layout="wide"
)
...
if df.empty:
    st.warning(
        "⚠️ No auction data available yet. The data pipeline is running in the background."
    )
    st.info("Please wait a few moments and refresh the page.")
    st.stop()
...
if pd.isna(min_date) or pd.isna(max_date):
    st.warning(
        "⚠️ Invalid date data detected. Please wait for the data pipeline to complete."
    )
    st.stop()

# Calculate default date range (last year of data)
...
if isinstance(date_range, tuple) and len(date_range) == 2:
    ...
    filtered_df = df[
        (df["auction_date"] >= start_datetime) & (df["auction_date"] < end_datetime)
    ].copy()
else:
...
if page == "📊 Overview":
    ...
    # Second row of metrics for Fed participation
    col1, col2, col3, col4 = st.columns(4)
    ...
    for sec_type in btc_data["security_type"].unique():
        ...
        btc_data.loc[mask, "ma_30"] = (
            btc_data.loc[mask, "bid_to_cover_ratio"].rolling(30, min_periods=1).mean()
        )
    ...
    st.caption(
        "Based on Brookings Institution framework for detecting Treasury market stress"
    )

    # Calculate yield spread (high - low)
    spread_data = filtered_df[
        (filtered_df["high_yield"].notna()) & (filtered_df["low_yield"].notna())
    ].copy()
    ...
    btc_trend["btc_ma_30"] = (
        btc_trend["bid_to_cover_ratio"].rolling(30, min_periods=1).mean()
    )
    ...
    if not bid_comp.empty:
        bid_comp["total_bids"] = (
            bid_comp["competitive_accepted"] + bid_comp["noncompetitive_accepted"]
        )
        bid_comp["comp_pct"] = (
            bid_comp["competitive_accepted"] / bid_comp["total_bids"] * 100
        )
        bid_comp["noncomp_pct"] = (
            bid_comp["noncompetitive_accepted"] / bid_comp["total_bids"] * 100
        )
    ...
    if fed_data.empty:
        ...
        st.caption(
            "Foreign and International Monetary Authorities - tracks foreign central bank demand"
        )

        fima_trend = fed_data[fed_data["fima_percentage"].notna()].sort_values(
            "auction_date"
        )
        ...
        st.caption(
            "System Open Market Account - Federal Reserve's own portfolio operations"
        )

        soma_trend = fed_data[fed_data["soma_percentage"].notna()].sort_values(
            "auction_date"
        )
        ...
        fed_combined["fed_total_pct"] = fed_combined["fima_percentage"].fillna(
            0
        ...
        fed_by_type = (
            fed_data.groupby("security_type")
            .agg(
                {
    ...
    stats = (
        filtered_df.groupby("security_type")["bid_to_cover_ratio"]
    ...
    # Discount/Investment Rate Analysis
    ...
    if not rate_data.empty:
        rate_data["discount_spread"] = (
            rate_data["high_discount_rate"] - rate_data["low_discount_rate"]
        )
    ...
    if len(available_cols) > 1:
        for sec_type in filtered_df["security_type"].unique():
            sec_data = filtered_df[filtered_df["security_type"] == sec_type][
                available_cols
            ].dropna()
    ...
    if fiscal_df.empty:
        ...
        fiscal_filtered = fiscal_df[
            (fiscal_df["date"] >= start_datetime) & (fiscal_df["date"] < end_datetime)
        ].copy()
        ...
        with col3:
            latest_index = (
                fiscal_filtered.iloc[0]["fiscal_policy_index"]
                if len(fiscal_filtered) > 0
                else 0
            )
        ...
        if rolling_window != "Daily (Raw)":
            ...
            plot_df["fiscal_policy_index_smooth"] = (
                plot_df["fiscal_policy_index"]
                .rolling(window=window_size, min_periods=1)
                .mean()
            )
            plot_df["tariff_fiscal_index_smooth"] = (
                plot_df["tariff_fiscal_index"]
                .rolling(window=window_size, min_periods=1)
                .mean()
            )
            plot_df["non_tariff_fiscal_index_smooth"] = (
                plot_df["non_tariff_fiscal_index"]
                .rolling(window=window_size, min_periods=1)
                .mean()
            )
        ...
        st.dataframe(fiscal_filtered[display_cols].head(20), use_container_width=True)
    ...
    if phrases_df.empty:
        ...
        fig2 = px.treemap(
            top_50, path=["phrase"], values="count", title="Phrase Frequency Treemap"
        )
        st.plotly_chart(fig2, use_container_width=True)
    ...
    if corr_df.empty:
        ...
        if rolling_window_corr != "Daily (Raw)":
            ...
            corr_plot_df["fiscal_policy_index_smooth"] = (
                corr_plot_df["fiscal_policy_index"]
                .rolling(window=window_size, min_periods=1)
                .mean()
            )
        ...
        fig.update_layout(
            ...
            yaxis=dict(title="Fiscal Policy Index", side="left"),
...
st.markdown("---")
st.caption(
    f"Data from {min_date.strftime('%Y-%m-%d')} to {max_date.strftime('%Y-%m-%d')} | Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M')}"
)
